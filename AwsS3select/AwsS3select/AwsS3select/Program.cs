...
namespace AwsS3select
{
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            _bucketName = "javasorn-s3select";
            _keyName = "S3Object.json";
            var stream = GetSelectObjectContentEventStream().Result;

            stream.ProgressEventReceived += Stream_ProgressEventReceived;
            stream.RecordsEventReceived += Stream_RecordsEventReceived;
            
            Console.Read();
        }

        private static void Stream_ProgressEventReceived(object sender, Amazon.Runtime.EventStreams.EventStreamEventReceivedArgs<ProgressEvent> e)
        {
        }

        private static void Stream_RecordsEventReceived(object sender, Amazon.Runtime.EventStreams.EventStreamEventReceivedArgs<RecordsEvent> e)
        {
            using (var rd = new StreamReader(e.EventStreamEvent.Payload))
            {
